...
function LoadFilesWithKeywords(aIndexFiles, aIndexFileTitles, aIndexStopWords, aKeywords, aFiles, aFileTitles, aFileKeywords)
{
    ...
    for (var nStopWord = 0; nStopWord < aKeywords.length; nStopWord++)
    {
       // find stop word in index
        // 1.6.2016 Antti A: Disabled the if clause in order to enable partial matching of the search query against the search index
        //if (aIndexStopWords["_" + aKeywords[nStopWord]] != undefined)
        
        // 31.5.2016 Antti A.: Loop through the search index in order to include partial matches of the query in the search results 
        for (var key in aIndexStopWords) {
            if (key.indexOf(aKeywords[nStopWord]) > -1) {

                // get indices of files containing word
                var strIndices = new String(aIndexStopWords[key]);
