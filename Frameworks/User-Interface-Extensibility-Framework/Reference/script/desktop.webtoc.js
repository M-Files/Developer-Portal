...
function syncToCNodeImplementation(anchor) {
    ...
    if (anchor.length && ((previouslySelectedNode != null && anchor.attr('data-node-index') != previouslySelectedNode.attr('data-node-index')) || previouslySelectedNode == null)) {
		...
		// 8.6.2016 Antti A.: If anchor position cannot be calculated (if the TOC is hidden, the anchor position cannot be calculated in Firefox nor in IE), wait for the TOC to be opened:
		if (anchor.offset().top == 0) {
			parent.$(parent.document).on('openTOC', function(event) {
